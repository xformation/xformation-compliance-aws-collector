...
import com.synectiks.aws.entities.lambda.ExecutionRole;
...
public class XformLambdaProcessor extends XformAwsProcessor {
	...
	protected LambdaClient getCloudObject() throws Exception {
		...
		LambdaClient awsLambdaClient = LambdaClient.builder().credentialsProvider(awsCredentialsProvider).region(getRegion()).build();
	...
	public List<XformLambda> getXformObject() throws Exception {
		...
		try {
			...
			for (FunctionConfiguration functionConfiguration : list) {
				...
				Optional<VpcConfigResponse> vpcConfig = functionConfiguration.getValueForField("VpcConfig", VpcConfigResponse.class);
				Optional<DeadLetterConfig> deadLetterConfig = functionConfiguration.getValueForField("DeadLetterConfig", DeadLetterConfig.class);
				Optional<EnvironmentResponse> environment = functionConfiguration.getValueForField("Environment", EnvironmentResponse.class);
				Optional<String> kMSKeyArn = functionConfiguration.getValueForField("KMSKeyArn", String.class);
				Optional<TracingConfigResponse> tracingConfig = functionConfiguration.getValueForField("TracingConfig", TracingConfigResponse.class);
				Optional<InstanceState> masterArn = functionConfiguration.getValueForField("MasterArn", InstanceState.class);
				...
				Optional<String> lastUpdateStatus = functionConfiguration.getValueForField("lastUpdateStatus", String.class);
				Optional<Boolean> lastUpdateStatusReasonCode = functionConfiguration.getValueForField("LastUpdateStatusReasonCode", Boolean.class);
				Optional<List> fileSystemConfigs = functionConfiguration.getValueForField("FileSystemConfigs", List.class);
				Optional<String> packageType = functionConfiguration.getValueForField("PackageType", String.class);
				Optional<IamInstanceProfile> imageConfigResponse = functionConfiguration.getValueForField("ImageConfigResponse", IamInstanceProfile.class);
				Optional<String> signingProfileVersionArn = functionConfiguration.getValueForField("SigningProfileVersionArn", String.class);
